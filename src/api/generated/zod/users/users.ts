...
export const ListUsersApiV1UsersGetQueryParams = zod.object({
  "page": zod.number().min(1).default(listUsersApiV1UsersGetQueryPageDefault).describe('Page number (starts from 1)'),
  "page_size": zod.number().min(1).max(listUsersApiV1UsersGetQueryPageSizeMax).default(listUsersApiV1UsersGetQueryPageSizeDefault).describe('Number of items per page'),
  "role": zod.union([zod.enum(['student', 'instructor', 'admin', 'manager']).describe('Enumeration of user roles within the ICPC backend system.\n\nDefines the hierarchical roles that control access permissions\nand determine what actions users can perform.\n\nAttributes:\n    student: Regular students who can participate in contests and join teams.\n    instructor: Instructors who can create and manage contests for their courses.\n    admin: System administrators with full access to all system features.\n    manager: Organizational managers with elevated permissions across contests.'),zod.null()]).optional().describe('Filter by user role'),
  "q": zod.union([zod.string(),zod.null()]).optional().describe('Search by name, email, or phone number'),
  "audience_ids": zod.union([zod.array(zod.uuid()),zod.null()]).optional().describe('Filter by audience IDs')
})
