...
export const RunCodeApiV1ExecutionRunPostBody = zod.object({
  "question_id": zod.uuid().describe('Question UUID to run code against'),
  "source_code": zod.string().min(1).max(runCodeApiV1ExecutionRunPostBodySourceCodeMax).describe('The source code to execute'),
  "language_id": zod.number().gt(runCodeApiV1ExecutionRunPostBodyLanguageIdExclusiveMin).describe('Judge0 language ID (e.g., 71 for Python)')
}).describe('Request to run code against test cases (practice\/feedback).\n\nStdin and expected outputs are fetched from non-hidden test cases.\nUser only provides code and language.')

export const RunCodeApiV1ExecutionRunPostResponse = zod.object({
